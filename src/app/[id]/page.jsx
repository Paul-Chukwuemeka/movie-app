...
const Page = () => {
  ...
  const id = movieId ? movieId : paramId;
        ...
        <div className="flex flex-col pb-10 gap-5 px-8">
          ...
          <div className="flex flex-col gap-2">
            ...
            <ul className="flex gap-2">
              {data &&
                data.genres.slice(0,10).map((genre, i) => {
                  return (
                    <li
                      key={i}
            ...
            <div className="flex gap-6 items-center snap-x snap-mandatory w-full movies overflow-x-scroll">
              {data &&
                data.credits?.cast?.map((item, i) => {
                  return (
                    <div key={i} className="flex w-60 max-md:w-30 shrink-0 snap-start flex-col items-center gap-3">
                      <Image
                        width={100}
                        height={100}
                      ...
                      <div className="text-center text-lg max-md:text-sm font-semibold">
                        <p>{item.name}</p>
                        <p className="text-gray-500 text-wrap">{item.character}</p>
                      </div>
                    </div>
                  );
                })}
